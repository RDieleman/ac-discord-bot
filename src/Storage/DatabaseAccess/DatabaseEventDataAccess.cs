...
namespace Storage
{
    public class DatabaseEventDataAccess : IEventDataAccess
    {
        ...
        public async Task<IEnumerable<Event>> GetGuildEventsAsync(int clanId)
        {
            ...
            var sql =
                "SELECT e.*, u.name FROM events e INNER JOIN users u ON e.user_id = u.id WHERE e.clan_id = @ClanId AND e.start_date > @minDate AND e.start_date < @maxDate AND e.end_date > @minDate AND e.end_date >= e.start_date;";
