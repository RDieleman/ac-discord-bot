    ...
    public sealed class CooldownAttribute : CheckBaseAttribute
    {
        ...
        public override async Task<bool> CanExecute(CommandContext ctx, bool help)
        {
            ...
            if (!result)
            {
                ...
                embed.Description = $"You can use this command again in `{Math.Ceiling((bucket.ResetsAt.ToUniversalTime() - DateTime.UtcNow).TotalSeconds)}` seconds.";
                _ = ctx.CommandsNext.Dependencies.GetDependency<IDiscordMessages>().SendAndDeleteMessageAsync(ctx.Channel.Id, "", embed);
                return false;
            }
